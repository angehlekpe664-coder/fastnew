...
const SUSPICIOUS_PDF_TOOLS: Array<{ pattern: RegExp; label: string }> = [
...
function pickPdfField(raw: string, key: string): string | null {
  const paren = raw.match(new RegExp(`/${key}\\s*\\(([^)]*)\\)`, "i"));
  if (paren?.[1]) return decodePdfString(paren[1]).trim() || null;
...
export function extractPdfMetaFromBuffer(buffer: Buffer): PdfMeta {
...
function parsePdfMetaDate(value: string | null): Date | null {
...
function mergePdfMeta(rawMeta: PdfMeta, pdfInfo?: Record<string, unknown>): PdfMeta {
  const info = pdfInfo ?? {};
  return {
    ...
    creationDate:
      rawMeta.creationDate ?? (typeof info.CreationDate === "string" ? info.CreationDate : null),
...
function checkSuspiciousEditor(meta: PdfMeta): IntegrityCheck {
...
function checkPdfStructure(meta: PdfMeta): IntegrityCheck {
  ...
  if (created && modified && modified.getTime() - created.getTime() > 3_600_000) {
    ...
    const trustedGenerator =
      /treasury|tresor|finances|dgtcp|itext|acrobat distiller|microsoft|chrome|wkhtml|reportlab/i.test(
        haystack
      );
...
export type IntegrityOptions = {
  mimeType: string;
  expectedTp?: string;
  checkPdfMeta?: boolean;
  checkSuspiciousEditors?: boolean;
  checkAmountConsistency?: boolean;
  checkTpConsistency?: boolean;
};

export function analyzeDocumentIntegrity(
  buffer: Buffer,
  rawText: string,
