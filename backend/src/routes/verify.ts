...
verifyRouter.get("/tp", async (req, res) => {
  ...
  if (filiere) list = list.filter((tp) => tp.filiere === filiere);
...
verifyRouter.get("/tp/:code", async (req, res) => {
  ...
  if (!tp) return res.status(404).json({ error: "Code TP inconnu." });
  return res.json(tp);
});
...
verifyRouter.post("/", upload.single("quittance"), async (req, res) => {
  try {
    ...
    const storagePath = await uploadQuittance(req.file.buffer, req.file.originalname, req.file.mimetype).catch(
...
verifyRouter.get("/status/:validationId", async (req, res) => {
  ...
  if (!data) return res.status(404).json({ error: "Identifiant introuvable." });
